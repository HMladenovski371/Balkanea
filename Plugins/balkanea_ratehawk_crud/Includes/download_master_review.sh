...
# Clean directory path (remove CR, LF, spaces)
...
echo "Starting master reviews download process for all languages..."
echo "Working directory: $CRUD_DATA_DIR"

# Define paths and URLs
REVIEWS_URL="https://api.worldota.net/api/b2b/v3/hotel/reviews/dump/"
...
process_language() {
    ...
    # Step 7: Clean + validate JSON
    echo "Cleaning and validating JSON file for $LANGUAGE..."
    if [ -f "$REVIEWS_JSON_FILE" ]; then
        sed -i 's/,,/,/g' "$REVIEWS_JSON_FILE"
        sed -i 's/,\([[:space:]]*\)}/\1}/g' "$REVIEWS_JSON_FILE"
        sed -i 's/,\([[:space:]]*\)]/\1]/g' "$REVIEWS_JSON_FILE"
...
for LANGUAGE in "${LANGUAGES[@]}"; do
    process_language "$LANGUAGE" || echo "Warning: Failed to process language $LANGUAGE"
    echo "----------------------------------------"
done
