...
class ImageInserter {
    ...
    private function insertImage(string $entity, string $imgUrl): int|null{
        try {
            $success = $this->wpdb->replace(
                $this->wpdb->prefix . 'hotel_images',
                array(
            ...
            if($success){
                ...
                $this->log->error('Image not inserted because of error: ' . $this->wpdb->last_error);
                return null;
            }
        }
        catch (\Throwable $e){
    ...
    public function insertImages(string $entity = 'hotel') {
        try {
            ...
            $imgId = $this->insertImage($entity, json_encode($imgs));
