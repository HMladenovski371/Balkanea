...
class Ratehawk_Admin_Page {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_ratehawk_clear_statuses', function() {
    ...
    wp_send_json_success(array('message' => 'All statuses cleared.'));
});
    ...
    public function handle_form_submissions() {
        ...
        if (isset($_POST['update_countries'])) {
            ...
            // Support multiple countries as array
            if ($update_type === 'specific') {
                $countries = array_map('trim', explode(',', $country_code));
            } else {
    ...
    /* =========================
       UPDATE + IMPORT (with multiple countries & .running files)
       ========================= */
    public function run_update_and_import($countries = array()) {
        if (!is_array($countries)) $countries = [];

        // If empty array, update all countries
        if (empty($countries)) $countries = ['all'];

        foreach ($countries as $country) {
            $timestamp = time();
            $running_file = sys_get_temp_dir() . "/ratehawk_{$country}_{$timestamp}.running";

            file_put_contents($running_file, "running");

            $cmd = "php " . escapeshellarg($this->plugin_path . "/process_hotels_automate.php") .
    ...
    public function ajax_check_status() {
        ...
        foreach ($status as $key => $proc) {
            $running_file = isset($proc['running_file']) ? $proc['running_file'] : '';
            ...
            if (!$is_running && $proc['status'] === 'running') {
                ...
                if (isset($proc['type']) && $proc['type'] === 'extract' && !empty($proc['country'])) {
...
public function ajax_stop_script() {
    ...
    if (empty($key) || !isset($status[$key])) {
        wp_send_json_error(array('message' => 'Invalid process key'));
    }
    ...
    if ($stopped) {
        wp_send_json_success(array('message' => 'Process stopped.'));
    } else {
        wp_send_json_error(array('message' => 'Failed to stop process.'));
    }
}
