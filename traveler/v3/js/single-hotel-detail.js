...
(function ($) {
    ...
    function loadPage(page, hotel_id, data) {
        ...
        getNonce("search_room").then(nonce => {
                if (!nonce) {
                    console.error("Failed to retrieve nonce.");
                    return;
                }
            $.ajax({
                url: `${current_domain_url}/wp-plugin/APIs/search_hotel.php`,
                type: 'GET',
                data: {
    ...
    async function isRateHawkHotel() {
        const currentUrl = window.location.href;
        const postName = currentUrl.split('/')[4];
        
        try {
            const response = await fetch(`${current_domain_url}/wp-plugin/APIs/is_provider_hotel.php?post_name=${postName}`);
...
function loadRoomsFromDB (ajax_url, data, body, loader, message) {
    ...
    function getParamFromUrl(param) {
        ...
        if (params.has(param)) {
            parameter_value = params.get(param);
        } else {
    ...
    async function orderBookingForm(order_data, data) {
        let is_provider = await isRateHawkHotel();
        
        if (!is_provider) return 'not-provider';
    
        const book_hash = await prebookRoom();
        
        console.log(book_hash);
    
        if (!book_hash) return 'book-hash-not-found';
    
        let HTML_price_element = document.querySelector("#st-content-wrapper > div.container > div > div.row > div.col-12.col-lg-4 > div.widgets > div > div.st-form-book-wrapper.relative > div.form-booking-price > span.price");
    
        let HTML_price = HTML_price_element ? HTML_price_element.innerHTML.split(' ')[1] : null;
    
        order_data['HTML-price'] = HTML_price;
        order_data['book_hash'] = book_hash;
        order_data['security'] = st_params._s;
        order_data['order_data'] = JSON.stringify(data);
        order_data['action'] = 'order_booking_form';
    ...
    function showNonClosableModal(message) {
    ...
    // Nikola JS functions START
    function prebookRoom() {
        const book_hash_URI = getParamFromUrl('book_hash');
    
        if (!book_hash_URI) {
            return;
        }
    
        return getNonce("order_booking_form").then(nonce => {
            if (!nonce) {
                console.error("Failed to retrieve nonce.");
                return;
            }
            ...
            return $.ajax({
                url: `${current_domain_url}/wp-plugin/APIs/order_booking_form.php`,
                method: 'post',
                data: {
                    'type': 'prebook',
                    'data': book_hash_URI,
                ...
                if (succ === "true") {
                    showNonClosableModal('The prices for your booking have changed. Please go back and select the room again.');
                    return null;
                } else {
                    return succ.book_hash;
                }
            }).catch(err => {
                console.log(err);
                showNonClosableModal('Oops something went wrong. Please go back and select the room again.');
                return null;
            });
        });
    }
    
    function displayMetaPolicy(metapolicy) {
    const container = $("#st-list-room");
    
    function createAccordionCard(id, title, content) {
        const card = document.createElement('div');
        card.className = title;

        card.innerHTML = `
                <div class="card-balkanea">
                  <h4 class="card-header-balkanea">
                    ${title}
                  </h4>
                  <div class="card-body-balkanea">
                ...
                <div style="margin: 30px 0 30px 0;" class="st-hr"></div>
        `;
        ...
        function mapToTable(data) {
            ...
            data.forEach(item => {
                ...
                keys.forEach(key => {
                    ...
                    if (isNaN(item[key])){
                        item_key = item[key].replace("-", " ");
                        item_key = item_key.replace("_", " ");
                    }
                    table += `<td>${item_key ?? 'N/A'}</td>`;
                });
                table += '</tr>';
            });
        ...
        Object.entries(metapolicy).forEach(([key, value], index) => {
            let content;
            let title = key;
            if (Array.isArray(value)) {
                content = mapToTable(value);
            ...
            if (content != 'No data'){
                const card = createAccordionCard(index, key.replace(/_/g, ' ').toUpperCase(), content);
                accordion.appendChild(card);
            }
        });
    
        container.after(accordion);
    }


    function generateMetapolicyStruct() {
        const currentUrl = decodeURIComponent(window.location.href);
        const hotel_id = document.querySelector('[name="room_parent"]');
        
        if (currentUrl.split('/')[3] != 'hotel' || !hotel_id){
            return;
        }
        
        let hotel_id_value = hotel_id.value;
        
        getNonce("query").then(nonce => {
                if (!nonce) {
                    console.error("Failed to retrieve nonce.");
                    return;
                }
            $.ajax({
                url: `${current_domain_url}/wp-plugin/APIs/request_query.php`,
                method: 'GET',
                data: {
                ...
                success: (succ) => {
                    ...
                    const parsedArray = JSON.parse(succ);
    
                    const metaValueString = parsedArray[0].meta_value;
                    
                    const metaPolicyStruct = JSON.parse(metaValueString);
                    
                    displayMetaPolicy(metaPolicyStruct);
                },
                error: (err) => {
    ...
    let SingleHotelDetail = {
        ...
        //         // Nikola - validate reservation guests START
        //         form.find('input[name="guest_name[]"]').each(function () {
        //             let fullName = $(this).val().trim();
        //             let nameParts = fullName.split(/\s+/);
                    
        //             let hasNumbers = /\d/.test(fullName);
                    
        //             if (nameParts.length == 1) {
        //                 $('div.message-wrapper').html('<p class="text-danger">Please enter first name and last name.</p>');
        //                 isError = true;
        //                 return false;
        //             }
                
        //             if (hasNumbers) {
        //                 $('div.message-wrapper').html('<p class="text-danger">Names cannot contain numbers.</p>');
        //                 isError = true;
        //                 return false;
        //             }
        //         });
        ...
        //         if (isError)
        //             return;
        //         // Nikola - validate reservation guests END
                
        //         // Nikola - Data prep for reservation request START
        //         var data = $('form.hotel-room-booking-form').serializeArray();
        //         var order_data = {};
        //         var loadingSubmit = form.find('button[name=submit]');
        //         let free_cancelation = getParamFromUrl('free_cancellation_before');
        ...
        //         });
        //         data.push({
        //             name: 'free_cancellation_before',
        //             value: free_cancelation
        //         });
        ...
        //                 bookingRes = await orderBookingForm(order_data, data);
                        
        //                 console.log(bookingRes);
        //                 var HTML_price_element = document.querySelector("#st-content-wrapper > div.container > div > div.row > div.col-12.col-lg-4 > div.widgets > div > div.st-form-book-wrapper.relative > div.form-booking-price > span.price");

        //                 var HTML_price = HTML_price_element ? HTML_price_element.innerHTML.split(' ')[1] : null;
                        
        //                 if (bookingRes == 'not-provider'){
        //                     if (res &&  res.status) {
        //                         if (res.redirect) {
        ...
        //                         }
        //                     }
        //                 }else if (bookingRes == 'book-hash-not-found'){
        //                     $('div.message-wrapper').html('<p class="text-danger"> Invalid hash or price please try again! </p>')
        //                     setTimeout( () => {
        //                         window.location.href = current_domain_url;
        //                         return;
        //                     }, 5000)
        //                 }
                        
        //                 if (bookingRes.success == false){
        //                     $('div.message-wrapper').html('<p class="text-danger"> Something went wrong please try again! </p>');
        //                     return;
        //                 }
        //                 console.log(bookingRes);
        //                 // TO DO: Observe the logic
        //                 // TASK: https://ohanaone.atlassian.net/browse/BAL-414
        //                 $(loadingSubmit).find('i.fa-spin').addClass("d-none");
        //                     if (bookingRes.data.message == 'error'){
        //                         $('div.message-wrapper').html('<p class="text-danger">Something went wrong please try again. </p>');
        //                         setTimeout( () => {
        //                             window.history.go(-1);
        //                         }, 2000)
        //                     }
        //                     else if (bookingRes.data.message == 'ok'){
        //                         window.location.href = bookingRes.data.redirect;
        //                     }
        //                     else if (res &&  res.status) {
        //                         if (bookingRes == 'not found' || res.redirect) {
        //                             window.location.href = res.redirect;
        //                         }
        //                     } else {
        //                         if (res.message && bookingRes.data.message == 'erorr') {
        //                             $('div.message-wrapper').html(res.message);
        //                         }
        //                     }
        //             },
        ...
        //         });
        //     });
        //     // Nikola - Data prep for reservation request END
        // },
        _bookingRoomAjax: function () {
            $('form.hotel-room-booking-form').on('click', 'button.btn-book-ajax', function (e) {
                var isError = false;
                ...
                // Making the request using only orderBookingForm
                // (async function() {
                //     try {
                //         $('div.message-wrapper').html("");
                        
                //         const bookingRes = await orderBookingForm(order_data, data);
                ...
                //         if (bookingRes == 'not-provider') {
                //             $('div.message-wrapper').html('<p class="text-danger">Service not available.</p>');
                //             return;
                //         }
                        
                //         if (bookingRes == 'book-hash-not-found') {
                //             $('div.message-wrapper').html('<p class="text-danger">Invalid hash or price please try again!</p>');
                //             setTimeout(() => {
                //                 window.location.href = current_domain_url;
                //             }, 5000);
                ...
                //         if (bookingRes.data && bookingRes.data.message == 'error') {
                //             $('div.message-wrapper').html('<p class="text-danger">Something went wrong please try again.</p>');
                //             setTimeout(() => {
                //                 window.history.go(-1);
                //             }, 2000);
                //         } else if (bookingRes.data && bookingRes.data.message == 'ok') {
                //             window.location.href = bookingRes.data.redirect;
                //         } else if (bookingRes.status && bookingRes.redirect) {
                //             window.location.href = bookingRes.redirect;
                //         } else if (bookingRes.message) {
                //             $('div.message-wrapper').html(bookingRes.message);
                //         }
                        
                //     } catch (error) {
                //         console.log(error);
                //         $('div.message-wrapper').html('<p class="text-danger">An error occurred. Please try again.</p>');
                //         $(loadingSubmit).find('i.fa-spin').addClass("d-none");
                //     }
                // })();
                // Nikola - Data prep for reservation request END
        ...
        _stcheckavailabilityHotel: async function (body) {
            await window.retrieveHotelPage();
           /* $('.form-check-availability-hotel .submit-group [name="submit"]', body).on('click', async function (ev) {

                ev.preventDefault();
                
                // Nikola - dynamic room loading START
                const $icon = $(this).find('span');
        
                if ($icon.length) {
                    $icon.fadeOut(100, function () {
                        $icon.attr('class', 'fa fa-spinner fa-spin').fadeIn(100);
                    });
                }
                ...
                if ($icon.length) {
                    $icon.fadeIn(100, function () {
                        $icon.attr('class', 'fa fa-spinner fa-spin').fadeOut(100);
                    });
                }
                // Nikola - dynamic room loading START
            });
