...
{
    if ($key) {
        // Our input field is always named "captcha_input" now
        $user_input = isset($_REQUEST['captcha_input']) ? strtolower(trim($_REQUEST['captcha_input'])) : '';

        if (empty($_SESSION[$key]) || $user_input !== $_SESSION[$key]) {
            return false;
        }
