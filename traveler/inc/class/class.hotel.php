    ...
    if ( !class_exists( 'STHotel' ) ) {
        ...
        public function order_booking_form(){
            try{
                ...
                if (STInput::request('room_id') == null &&  STInput::request('book_hash') == null){
                    error_log("public_html/wp-content/themes/traveler/inc/layouts/modern/class/class.hotel.php:Line 183");
                ...
                error_log("Free cancellation before: " . $free_cancellation_before);
                
                $price_expode_currency = explode(' ', $price);
        
                // $price = $this->ConvertCurrency($price_expode_currency);
        
                error_log("Price After Convert: ");
                error_log($daily_price);
                ...
                setcookie($cookie_name, $cookie_value, time() + 1200, "/", "", true, false);
                setcookie($cookie_value . '_order_data', $order_data, time() + 1200, "/", "", true, false);
                setcookie($cookie_value . '_free_cancellation_before', $free_cancellation_before, time() + 1200, "/", "", true, false);
                ...
                if ($response === false)
                    ...
                    if ($response['status'] == 'ok'){
                        ...
                        wp_send_json_success(['message' => $response['status']]);
                    }
                    else{
                        error_log("public_html/wp-content/themes/traveler/inc/layouts/modern/class/class.hotel.php:Line 200");
                        error_log(print_r($response, true));
                        wp_send_json_success(['message' => $response['status']]);
                    }
                }
                }catch (\Exception $ex){
                    error_log("public_html/wp-content/themes/traveler/inc/layouts/modern/class/class.hotel.php:206");
                    error_log($ex->getMessage());
                    wp_send_json_error(['message' => 'error']);
                    exit();
                }
