 ...
 * */
    class STCart
    {
        ...
        static function init()
        {
            ...
            // Nikola 30.12.2024
            add_filter('wp_ajax_update_cart_price', [__CLASS__, 'update_cart_price']);
            add_filter('wp_ajax_nopriv_update_cart_price', [__CLASS__, 'update_cart_price']);
            // Nikola End 30.12.2024
        }
        // Nikola Start function 30.12.2024
        static function update_cart_price() {
            if (!class_exists('WC_Product')) {
                wp_send_json_error(['message' => 'WooCommerce class not found.']);
            }
        
            try {
                WC()->cart->empty_cart();
        
                $full_price = isset($_POST['full_price']) ? floatval($_POST['full_price']) : 0;
                $fees = isset($_POST['fees']) ? $_POST['fees'] : [];
                $free_cancellation_before = isset($_POST['free_cancellation_before']) ? $_POST['free_cancellation_before'] : '';
                $daily_room_price = isset($_POST['daily_room_price']) ? floatval($_POST['daily_room_price']) : 0;
        
                if ($full_price <= 0) {
                    wp_send_json_error(['message' => 'Invalid price provided.']);
                    return;
                }
        
                if (get_woocommerce_currency() == "MKD"){
                    WC()->session->set('full_price',  $full_price / 61.53 );
                    WC()->session->set('daily_room_price',  $daily_room_price / 61.53 );
                }else{
                    WC()->session->set('full_price', $full_price);
                    WC()->session->set('daily_room_price', $daily_room_price );
                }
                
                WC()->session->set('fees', $fees);
                    
                // BAL-640 START
                if (!empty($free_cancellation_before))
                    WC()->session->set('free_cancellation', $free_cancellation_before);
                // BAL-640 END
                ...
                error_log("Error message in /home/balkanea/public_html/wp-content/themes/traveler/inc/layouts/modern/class/class.cart.php: 103");
                error_log($ex->getMessage());
                exit();
            }
        }
        // Nikola End function 30.12.2024
