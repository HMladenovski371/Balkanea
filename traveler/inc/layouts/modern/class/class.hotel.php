...
if (!class_exists('STHotel')) {
    class STHotel extends TravelerObject
    {
        ...
        function init()
        {
            ...
            add_action('wp_ajax_order_booking_form', [$this, 'order_booking_form']);
            add_action('wp_ajax_nopriv_order_booking_form', [$this, 'order_booking_form']);
        }
        
        public function UpdatePrice ($price, $room_id, $prefix, $wpdb) {
            try {
        ...
        public function ConvertCurrency($price_expode_currency){
        ...
        public function order_booking_form() {
        try {
            $this->include_wp_load();
            global $wpdb;
            $prefix = $wpdb->prefix;
    
            if (!class_exists('WC_Product')) {
                $this->send_json_error('WooCommerce not loaded');
            }
    
            $price = WC()->session->get('full_price');
            $daily_price = WC()->session->get('daily_room_price');
    
            if (!$price || !$daily_price) {
                $this->send_json_error('Price data missing from session');
            }
    
            $room_id = STInput::request('room_id');
            $book_hash = STInput::request('book_hash');
            $order_data = STInput::request('order_data');
            $free_cancellation_before = STInput::request('free_cancellation_before');
            $HTML_price = STInput::request('HTML-price');
    
            if (!$room_id || !$book_hash) {
                $this->log_error("Data is missing: room_id or book_hash");
                $this->send_json_error('Missing booking data');
            }
            
            if (get_woocommerce_currency_symbol() == 'MKD'){
                $price_compare = round( $price * 61.53 );
            }else{
                $price_compare = round($price, 2);
            }
            
            if ($HTML_price != $price_compare){
                $this->log_error("Price not same as HTML");
                $this->send_json_error('An error occurred');
            }
            
            $this->UpdatePrice($daily_price, $room_id, $prefix, $wpdb);
    
            $partner_order_id = time();
            $this->set_booking_cookies($partner_order_id, $order_data, $free_cancellation_before);
    
            $response = $this->send_booking_request($partner_order_id, $book_hash);
            
            if ($response && isset($response['status']) && $response['status'] == 'ok') {
                $payment_data = json_encode($response['data']['payment_types'][0]);
                setcookie($partner_order_id, $payment_data, time() + 600, "/", "", true, false);
              //Hristijan Changes
              //  wp_send_json_success(['message' => 'ok', 'redirect' => "https://staging.balkanea.com/cart/"]);
              wp_send_json_success([    'message' => 'ok',    'redirect' => home_url('/cart/')]);
            } else {
                $this->log_error("API error: " . print_r($response, true));
               // wp_send_json_success(['message' => 'error', 'redirect' => "https://staging.balkanea.com/cart/"]);
               wp_send_json_success([    'message' => 'error',    'redirect' => home_url('/cart/')]);
            }
        } catch (\Exception $ex) {
            $this->log_error("Exception: " . $ex->getMessage());
            $this->send_json_error('An error occurred');
        }
    }

    /**
     * Includes wp-load.php for WordPress functionality.
     */
    private function include_wp_load() {
    ...
    private function log_error($message) {
        error_log("public_html/wp-content/themes/traveler/inc/layouts/modern/class/class.hotel.php: " . $message);
    }
    ...
    /**
     * Sets booking-related cookies.
     */
    private function set_booking_cookies($partner_order_id, $order_data, $free_cancellation_before) {
        setcookie("partner_order_id", $partner_order_id, time() + 1200, "/", "", true, false);
        setcookie("{$partner_order_id}_order_data", $order_data, time() + 1200, "/", "", true, false);
        setcookie("{$partner_order_id}_free_cancellation_before", $free_cancellation_before, time() + 1200, "/", "", true, false);
    }
    
    /**
     * Sends a booking request to the external API.
     */
    private function send_booking_request($partner_order_id, $book_hash) {
        $path = $_SERVER['DOCUMENT_ROOT'];
        $config = include "{$path}/wp-plugin/config.php";
        ...
        error_log(print_r($body_data,true));
