...
<div class="<?php echo esc_attr( $class_wrapper ); ?> booking-item-payment">
	...
	<table class="shop_table booking-item-payment">
		<tfoot>
			...
			$full_price = WC()->session->get('full_price', 0); // Nikola display dynamic price WC session
			...
			<tr class="cart-subtotal">
				...
				<td><?php 
				    // Nikola display dynamic price WC session START
    				if ($full_price != null && $full_price > 0){
    				    echo TravelHelper::format_money($full_price);
    				}else{
    				    wc_cart_totals_order_total_html();
    				}
    				// Nikola display dynamic price WC session END
				?></td>
			</tr>
            ...
            <tr class="tax-info">
                <th><?php esc_html_e('Tax Information', 'woocommerce'); ?></th>
                <td>
                    <?php 
                    $tax_totals = WC()->cart->get_tax_totals(); // Get an array of tax totals
                    
                    if (!empty($tax_totals)) {
                        foreach ($tax_totals as $code => $tax) {
                            // Display each tax label and amount, including fees
                            echo sprintf(
                                '<div>%s: %s</div>',
                                esc_html($tax->label), // Tax name/label
                                wc_price($tax->amount) // Tax amount
                            );
                        }
                    } else {
                        echo esc_html__('No taxes applied', 'woocommerce');
                    }
                    ?>
                </td>
            </tr>
            <?php 
                // Nikola display fees from WC session START
                $free_cancellation = WC()->session->get('free_cancellation', '');
                ...
                if (is_array($fees)) {
                    ...
                    $fees_decode = json_decode($fees, true);
                ...
                if (!empty($free_cancellation)): ?>
                    <tr class="fee">
                        ...
                        <td><?php echo esc_html($free_cancellation); ?></td>
                    </tr>
                <?php endif; ?>
                <!-- BAL-640 END -->
                
            <?php    
                if (is_array($fees_decode) && count($fees_decode) > 0): ?>
                    <tr>
                        <td colspan="2">
                            <p class="text-danger blockquote-footer mt-1">These additional taxes are not included in the price</p>
                        </td>
                    </tr>
                    <?php foreach ($fees_decode as $key => $value): ?>
                        <tr class="fee">
