...
<div class="row st-woo-cartpage">
    <div class="col-md-8 col-left">
		<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
			<h2 class="st-woo-heading"><?php echo __('Order review', 'traveler'); ?></h2>
							...
							<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
								...
								<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'traveler' ); ?>">
									<span class="booking-item-price">
										<?php
    										if (WC()->session->get('full_price') != null && WC()->session->get('full_price') > 0){
        										echo TravelHelper::format_money(WC()->session->get('full_price'));
    										}else{
    										    if (get_woocommerce_currency() == 'MKD') {
                                                    $raw_price = $_product->get_price();
                                                    $converted_price = $raw_price * 61.53;
                                                
                                                    echo apply_filters(
                                                        'woocommerce_cart_item_price',
                                                        wc_price($converted_price, ['currency' => 'MKD']),
                                                        $cart_item,
                                                        $cart_item_key
                                                    );
                                                } else {
    ...
    <div class="col-md-4 col-right">
        <h2 class="st-woo-heading"><?php _e( 'Order totals', 'traveler' ); ?></h2>
